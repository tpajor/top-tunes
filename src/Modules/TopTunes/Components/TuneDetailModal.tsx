...
class TuneDetailModal extends React.PureComponent<IProps, IState> {
  ...
  componentDidMount() {
    if (!this.props.tune) {
      this.props.history.push(urls.reactRouter.topTunes);
    }
  }

  private handleClosingAnimation = (event: React.MouseEvent<HTMLElement>) => {
    event.preventDefault();

    this.setState({ isClosing: true });

    setTimeout(() => this.props.history.push(urls.reactRouter.topTunes), 500);
  }
    ...
    return (
      <div className={`container container--modal-background ${this.state.isClosing ? 'slide-out' : 'slide-in'}`}>
        <div className='container container--modal-main'>
          {this.props.tune ? this.getMainViewJSX() : <NoResults />}
        </div>
      </div>
    );
  }
